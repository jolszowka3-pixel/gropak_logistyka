...
PALLET_H = 144 # Wysokość samej palety EURO
...
with st.sidebar:
    ...
    if tryb == "📦 Paczka Kurierska":
        ...
        h_max = st.number_input("Maks. wysokość CAŁKOWITA (mm):", 200, 2500, 1600, help="Obejmuje paletę (144mm) + towar.")
...
def optymalizuj_palete_plecakowa(L, W, H, h_max):
    PL, PW = 1200, 800
    available_h = h_max - PALLET_H # Towar tylko powyżej palety
...
if tryb == "📦 Paczka Kurierska":
    ...
    if res:
        ...
        with c1:
            ...
            st.write(f"- Ułożenie: {rl}x{rw} mm (H: {rh})")
    ...
    if total > 0:
        with c1:
            ...
            st.write(f"Paleta: {PALLET_H} mm | Towar: {h_max - PALLET_H} mm")
            st.divider()
    ...
    else: st.error("❌ Karton za duży lub za mała wysokość całkowita!")
