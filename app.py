...
with st.sidebar:
    ...
    L = st.number_input("Długość (cm)", value=int(default_vals["L"]) if default_vals["L"] > 0 else 60)
    W = st.number_input("Szerokość (cm)", value=int(default_vals["W"]) if default_vals["W"] > 0 else 40)
    H = st.number_input("Wysokość (cm)", value=int(default_vals["H"]) if default_vals["H"] > 0 else 20)
    weight = st.number_input("Waga (kg)", value=float(default_vals["weight"]) if default_vals["weight"] > 0 else 5.0)
...
def rysuj_paczke(orig_dims, n_sztuk, axis_index, label, color):
    ...
    for s in range(1, n_sztuk):
        ...
        if axis_index == 0:
            temp_v = [[shift,0,0], [shift,W_f,0], [shift,W_f,H_f], [shift,0,H_f], [shift,0,0]]
        elif axis_index == 1:
            temp_v = [[0,shift,0], [L_f,shift,0], [L_f,shift,H_f], [0,shift,H_f], [0,shift,0]]
        else:
            temp_v = [[0,0,shift], [L_f,0,shift], [L_f,W_f,shift], [0,W_f,shift], [0,0,shift]]
    ...
    fig.add_trace(go.Scatter3d(
        ...
        mode='lines', line=dict(color='#222', width=4)
    ))
...
for n in range(1, max_sztuk + 1):
    opts = [
        (L*n, W, H, 0, f"Spięte wzdłuż ({n}xL)"), 
        (L, W*n, H, 1, f"Spięte obok siebie ({n}xW)"), 
        (L, W, H*n, 2, f"Spięte w pionie ({n}xH)")
    ]
